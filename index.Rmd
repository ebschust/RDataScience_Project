...
In order to observe any correlation between the number of graptolite synonymies and temporal factors or the naming author, the data is first filtered and then the number of instances per year or author are counted. These are displayed in interactive tables.
```{r,eval=FALSE}
...
synGrap_authoryear=
  ...
  summarise(n=n())
##2677 for 734 author/year combos
datatable(synGrap_authoryear,options=list(pageLength=10))
```

These three analyses are statistically tested for significance by applying two chi-squared tests. The first tests the null hypothesis that the probabilities of all the counts per category are equal. The second tests the null hypothesis that the observed and expected numbers are equal (Pearson's chi-squared test).
```{r, eval=FALSE}
# Per year
chisq.test(synGrap_yearcount$n)

synGrap_year_expct=mutate(synGrap_yearcount,expected=rep(15,length(synGrap_yearcount$n)))
chisq.test(rbind(synGrap_year_expct$n,synGrap_year_expct$expected))

# Per author
chisq.test(synGrap_authorcount$n)

synGrap_author_expect=mutate(synGrap_authorcount,expected=rep(6,length(synGrap_authorcount$n)))
chisq.test(rbind(synGrap_author_expect$n,synGrap_author_expect$expected))

# Per author per year
chisq.test(synGrap_authoryear$n)

synGrap_authoryear_expect=mutate(synGrap_authoryear,expected=rep(4,length(synGrap_authoryear$n)))
...
```{r, fig.cap="Map of graptolite locations grouped by decade", fig.height=3, fig.width=6, message=FALSE, warning=FALSE,echo=FALSE}
...
```{r, fig.cap="Animation of graptolite synonymies per year", fig.height=3, fig.width=6, message=FALSE, warning=FALSE, echo=FALSE}
worldmap=borders("world",colour="gray50",fill="white")
mapWorld=ggplot()+worldmap
