...
The final analysis is based on the comparison of the number of graptolites that have been renamed, or have synonyms, versus graptolites that are the senior synonym, which are species that have never been renamed or are new names of older species. These are grouped by naming author and the year they were named. Using a single chi-squared test, these two categories of graptolites are tested to determine if they are statistically different from each other by using a Monte Carlo simulation using 1000 replicates.
```{r,eval=FALSE}
nonsynGrap=
...
Finally, the number of renamed graptolites versus the number of graptolites that are a senior synonym are plotted as a cumulative sum to visually determine how the number of graptolites requiring a change in name has changed through time.
```{r, eval=FALSE}
ggplot()+
...
In addition to geography, graptolite synonymies could be correlated to the year the species were named or the naming author. This is analyzed by grouping the graptolite synonymies by year, by author, and by author and year (Tables 1, 2, and 3).
...
The above analyses raise the question if there is a significant difference between the number of renamed graptolites and the number of senior synonyms when grouped by author and year (Table 4).
...
chisq.test(Grap_authoryear$`n senior.synonym`,Grap_authoryear$`n renamed`,simulate.p.value = T,B=1000)
```
The chi-squared test has a p-value of 0.000999 indicating a statistical difference between the number of renamed graptolites and senior synonyms when grouped by author and year.  

Finally, the cumulative sum of the number of renamed graptolites and senior synonyms are plotted by date (Figure 4).
...
The line graph shows that the number of renamed graptolites and the number of senior synonyms follows the same line until about the 1930s. At that point, the two groups begin to diverge, with the number of senior synonyms continuing to grow while the number of renamed graptolites begins to level off in the early 2000s.
