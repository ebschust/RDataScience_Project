...
Load required packages in a code chunk (you may need to install some packages):
...
The data is contained in two comma separated files (CSV) that will need to be loaded into Rstudio from the working directory. As such, it is important to make sure that the working directory contains both of the Taxonomic Dictionary and the section data.
```{r}
#Load Taxonomic Dictionary

myDict=read.csv("Taxonomic dictionary Oct3'17_mod.csv",header=T,sep=",")

#Load section data

mySect=read.csv("sections_table format.csv", header=T, sep =",")
```
