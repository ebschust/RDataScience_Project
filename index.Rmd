...
The final analyses are based on the comparison of the number of graptolites that have been renamed, or have synonyms, versus graptolites that are the senior synonym, which are species that have never been renamed or are new names of older species. These are grouped by naming author and the year they were named. Using a chi-squared test, these two categories of graptolites are tested for statistical difference with a Monte Carlo simulation using 1000 replicates.
```{r,eval=FALSE}
nonsynGrap=
...
```{r,echo=FALSE, fig.cap="Table 4. Number of renamed graptolites versus senior synonyms grouped by author and naming year."}
nonsynGrap=
  filter(myDict,senior.synonym=="*")
...
```{r,echo=FALSE, fig.cap="Figure 4. Line graph of the cumulative sum of the number of renamed graptolites versus the number of senior synonyms over time."}
ggplot()+
  geom_line(data=synGrap_yearcount,aes(x=as.numeric(as.character(date)), y=cumsum(n)))+
  geom_line(data=nonsynGrap_year,aes(x=as.numeric(as.character(date)), y=cumsum(n)),col="red")
```
